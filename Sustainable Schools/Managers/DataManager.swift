...
class DataManager {
    ...
    func getWeeklyQuestions(completion: (Data) -> Void){
        ...
        let url = URL(string: Constants.Config.URLS.jsonQuestionsURL )
    ...
    func updatePoints(pointValue: Int){
        ...
        let values = [Constants.Config.DatabaseStuffEeeek.Keys.totalPoints: self.newTotalPoints, Constants.Config.DatabaseStuffEeeek.Keys.energyPoints: pointValue] as [String : Any]
        usersDBRef.updateChildValues(values, withCompletionBlock: { (err, ref) in
            if err != nil {
                print(err ?? Constants.Messages.Error.databaseError)
                return
            }
        })
